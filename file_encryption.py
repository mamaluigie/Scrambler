        ...
        try:
            ...
            with open(file_path, "rb") as encrypted_file:
                ...
                temp_path = os.path.join(
                    ...
                    os.path.split(file_path)[1].replace("_encrypted", "", 1),
                )
        ...
        if mode == "AES":
            ...
            with open(file_path, "rb") as unencrypted_file:
                ...
                with open(
                    os.path.join(
                        os.path.split(file_path)[0],
                        os.path.split(file_path)[1] + "_encrypted",
                    ),
                    "wb",
                ) as encrypted_file:
    ...
    for x in os.listdir(directory):
        if ("fun-scripts" not in directory) and ("fun-scripts" not in x):
            if os.path.isdir(os.path.join(directory, x)):
                ...
                directory_encrypt(
                    os.path.join(directory, x), key_size, mode, pickle_key
                )
...
def encrypt(directory, key_path, mode, key_size, file_path):
    ...
    if (directory != None) and (file_path != None):
        ...
        if key_path == None:
            ...
            key = load_key(
                fd.askopenfilename(
                    title="Select key", initialdir=os.path.split(file_path)[0]
                )
            )
        else:
...
def decrypt(directory, key_path, mode, file_path):
    ...
    # for file encryption
        ...
        if key_path == None:
            key_path = fd.askopenfilename(
                title="Select the key", initialdir=os.path.split(file_path)[0]
            )
